...
class ResBlock_do_woFECB_bench(nn.Module):
    def __init__(self, out_channel):
        super(ResBlock_do_woFECB_bench, self).__init__()
        self.main = nn.Sequential(
            BasicConv_do(out_channel, out_channel, kernel_size=3, stride=1, relu=True)
        )
    def forward(self, x):
...
class ResBlock_do_woFECB_bench_eval(nn.Module):
    def __init__(self, out_channel):
        super(ResBlock_do_woFECB_bench_eval, self).__init__()
        self.main = nn.Sequential(
            BasicConv_do_eval(out_channel, out_channel, kernel_size=3, stride=1, relu=True)
        )
    def forward(self, x):
